...
export function createTurnContract(input: string, now = Date.now()): TurnContract {
  return { claims: extractObservableClaims(input), hints: [], createdAt: now };
}

export function isEmptyContract(contract: TurnContract | undefined): boolean {
 ...
 */
export function addProbeHint(contract: TurnContract, hint: ProbeHint): boolean {
  if (!contract.claims.some((claim) => claim.id === hint.claim_id)) return false;
  const sanitized: ProbeHint = { claim_id: hint.claim_id };
  ...
  if (existing === -1) contract.hints.push(sanitized);
  else contract.hints[existing] = sanitized;
  return true;
}

function describeClaim(claim: ObservableClaim): string {
  const target =
    claim.target_description === undefined ? "" : ` (${claim.target_description})`;
  switch (claim.kind) {
 ...
 */
export function renderContractContext(
  contract: TurnContract | undefined,
): string | undefined {
  if (contract === undefined || contract.claims.length === 0) return undefined;
  ...
  if (estimateTokens(text) > HARD_TOKEN_LIMIT) return undefined;
  return text;
}
