...
export async function runCounterfactual(
  ...
  if (changedTests.length === 0) return null;

  const baseEvidence = {
    candidate_test_files: changedTests,
    command: null,
  ...
  const portable = changedTests.filter((file) => candidateFiles.includes(file));
  ...
  try {
    ...
    return {
      ...baseEvidence,
      ...resultClassification(baselineResult, candidateResult),
