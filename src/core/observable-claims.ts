import type { ObservableClaim, ObservableClaimKind } from "./types.js";
...
const REMOVE_VERB =
  /\b(?:remove|delete|drop|strip|get\s+rid\s+of|eliminate|take\s+out|kill)\b/i;
const ADD_VERB =
  /\b(?:add|show|display|render|insert|say|write|include|put|set\s+the\s+text|change\s+(?:it\s+)?to)\b/i;
...
const NON_COPY_LITERAL =
  /^(?:[./~]|https?:)|\.(?:ts|tsx|js|jsx|mjs|cjs|json|md|css|scss|html|vue|svelte|py|go|rs|ya?ml|toml|lock)$|^[a-z0-9_-]+\/[a-z0-9_./-]+$/i;
...
function isCopyLiteral(value: string): boolean {
  if (value.length === 0 || value.length > MAX_LITERAL_LENGTH) return false;
  ...
  if (/^\d+(?:\.\d+)?(?:px|rem|em|vh|vw|%)$/i.test(value)) return false;
  return true;
}

function firstRoute(text: string): string | undefined {
  ROUTE.lastIndex = 0;
...
function targetDescription(context: string): string | undefined {
  const match = context.match(
    /\b(?:the\s+)?([a-z][a-z0-9-]*(?:\s+[a-z][a-z0-9-]*){0,2})\s+(?:background|colour|color|text|width|height|size|to|should)\b/i,
  );
...
function quotedTextClaims(text: string): Draft[] {
  const drafts: Draft[] = [];
  QUOTED.lastIndex = 0;
  for (const match of text.matchAll(QUOTED)) {
    const literal = match[1] ?? match[2] ?? match[3] ?? "";
    const value = normaliseWhitespace(literal);
    if (!isCopyLiteral(value)) continue;
...
function styleClaims(text: string): Draft[] {
  const drafts: Draft[] = [];
  for (const match of text.matchAll(HEX_COLOUR)) {
    ...
    if (described !== undefined) draft.targetDescription = described;
    drafts.push(draft);
  }
...
function visibilityClaims(text: string): Draft[] {
  const drafts: Draft[] = [];
  // Only an explicit hide/show of a *named* element counts. "hide it" does not.
  const hide = text.match(
    /\b(?:hide|conceal)\s+the\s+([a-z][a-z0-9-]*(?:\s+[a-z][a-z0-9-]*){0,2})\b/i,
  );
  ...
  const show = text.match(
    /\b(?:reveal|unhide)\s+the\s+([a-z][a-z0-9-]*(?:\s+[a-z][a-z0-9-]*){0,2})\b/i,
  );
 ...
 */
export function extractObservableClaims(input: string): ObservableClaim[] {
  ...
  for (const draft of drafts) {
    ...
    if (route !== undefined) claim.route_hint = route;
    claims.push(claim);
