...
function unavailable(detail, reportPath) {
    return {
...
function malformed(detail, reportPath, reportHash) {
    return {
        provider: VISUAL_QA_PROVIDER,
        status: "MALFORMED",
        subject_bound: false,
...
export async function loadVisualQaEvidence(options) {
    ...
    try {
        ...
        if (code === "ENOENT")
            return unavailable("visual-qa report not found", reportPath);
        return unavailable(`visual-qa report unreadable: ${error instanceof Error ? error.message : "unknown error"}`, reportPath);
    }
    ...
    const limitReason = typeof report.coverage?.limit_reason === "string"
        ? report.coverage.limit_reason
        : null;
    if (verdict !== "PASS" && verdict !== "FAIL" && verdict !== "UNPROVEN")
        return malformed("visual-qa report is missing a known verdict", reportPath, reportHash);
    if (verdict === "FAIL") {
        return {
            provider: VISUAL_QA_PROVIDER,
            status: "FAIL",
    ...
    return {
        ...
        report_path: reportPath,
        issue_count: issueCount(report),
        detail: options.subjectBound === true
            ? "visual-qa PASS with bound subject"
            : "visual-qa PASS without a bound subject",
    };
}
//# sourceMappingURL=external-evidence.js.map