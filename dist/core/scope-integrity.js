...
const PACKAGE_BUILD_CONFIG = /(^|\/)(package\.json|pyproject\.toml|Cargo\.toml|go\.mod|composer\.json|Gemfile|Makefile|CMakeLists\.txt|Dockerfile|tsconfig(?:\.[^/]*)?\.json|vite\.config\.[^/]+|webpack\.config\.[^/]+|rollup\.config\.[^/]+|build\.gradle(?:\.kts)?|pom\.xml)$/i;
const TEST_PATH = /(^|\/)(test|tests|__tests__|spec)(\/|$)|(?:^|\/)[^/]+\.(?:test|spec)\.[^/]+$/i;
const MIGRATION_PATH = /(^|\/)(migrations?|db\/migrate|alembic\/versions)(\/|$)/i;
const GENERATED_PATH = /(^|\/)(dist|build|coverage|generated|gen|vendor|\.next|target)(\/|$)|(?:\.min\.(?:js|css)|\.generated\.[^/]+|\.g\.(?:cs|ts))$/i;
const GENERATED_MARKER = /(?:code generated|auto[- ]generated|generated file|do not edit)/i;
const BINARY_EXTENSION = /\.(?:png|jpe?g|gif|webp|ico|pdf|zip|gz|tar|7z|woff2?|ttf|eot|class|jar|so|dylib|dll|exe|bin|wasm)$/i;
const SECRET_LIKE_PATH = /(^|\/)(?:\.env(?:\.[^/]+)?|id_(?:rsa|dsa|ecdsa|ed25519)(?:\.pub)?|credentials?\.(?:json|ya?ml|toml)|secrets?\.(?:json|ya?ml|toml)|service[-_]?account\.json|[^/]+\.(?:pem|p12|pfx|key|keystore))$/i;
const SECRET_TEMPLATE_PATH = /(^|\/)\.env\.(?:example|sample|template)$/i;
function signal(severity, code, file, observed, why, evidence = []) {
    return {
        severity,
        code,
        file,
...
async function diskVersion(path) {
    try {
...
async function gitVersion(root, ref, path) {
    try {
...
function observedChange(subject, added, deleted) {
    if (added)
...
function dependencyEntries(path, source) {
...
function safeDependencySpecifier(value) {
    return /^[~^<>=*v0-9.xX|& -]+$/.test(value)
        ? value
        : "<non-version specifier redacted>";
}
function publicApiLines(path, lines) {
...
function inspectChange(change) {
    ...
    if (addedFile && SECRET_LIKE_PATH.test(path) && !SECRET_TEMPLATE_PATH.test(path))
        signals.push(signal("FAIL", "SCOPE_SECRET_LIKE_FILE_ADDED", path, "secret-like filename added", "The added path matches a deterministic credential or secret filename pattern; file contents were not inspected or emitted."));
    if (addedFile && isBinary(after.bytes, path))
        signals.push(signal("WARNING", "SCOPE_BINARY_ADDED", path, "binary file added", "The added file has a known binary extension or contains a NUL byte, so line-level scope review is unavailable."));
    if (LOCKFILES.has(path.split("/").at(-1) ?? ""))
        signals.push(signal("INFORMATION", "SCOPE_LOCKFILE_CHANGED", path, observedChange("lockfile", addedFile, deletedFile), "The path is a recognized dependency lockfile. This records the observed change without judging whether it was needed."));
    ...
    if (generated && !LOCKFILES.has(path.split("/").at(-1) ?? ""))
        signals.push(signal("WARNING", "SCOPE_GENERATED_FILE_MODIFIED", path, observedChange("recognizable generated file", addedFile, deletedFile).replace(" changed", " directly modified"), "The path or leading file marker identifies generated/build output. The observation does not prove the edit was improper."));
    if (PACKAGE_BUILD_CONFIG.test(path))
        signals.push(signal("WARNING", "SCOPE_PACKAGE_BUILD_CONFIG_MODIFIED", path, observedChange("package/build configuration", addedFile, deletedFile), "The path is a recognized package manifest or build configuration file."));
    if (MIGRATION_PATH.test(path) && before.exists && after.exists)
        signals.push(signal("WARNING", "SCOPE_MIGRATION_MODIFIED", path, "existing migration modified", "A file under a recognized migration path changed after already existing in the baseline."));
    if (!deletedFile && !isBinary(after.bytes, path)) {
        ...
        if (apiChanges.added.length > 0 || apiChanges.removed.length > 0)
            signals.push(signal("WARNING", "SCOPE_PUBLIC_API_CHANGED", path, "detectable public API declarations changed", "Added or removed declarations match exported/public syntax. Implementation-only value and body changes are ignored; compatibility impact is not inferred.", [
...
async function secretLikeFiles(root) {
    const matches = [];
    let visited = 0;
    async function walk(relative) {
        if (visited >= MAX_SECRET_SCAN_FILES)
            return;
        let entries;
        try {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (!SCAN_EXCLUDED.has(entry.name))
                    await walk(path);
            }
...
export async function analyzeScopeIntegrity(options) {
    ...
    const secretCandidates = baselineDirectory === undefined ? [] : await secretLikeFiles(options.root);
    const paths = [...new Set([...options.changedFiles, ...secretCandidates])].sort((left, right) => left.localeCompare(right));
    ...
    if (baselineDirectory === undefined &&
        ...
        return {
            ...
            task_touched: paths,
            signals: [],
            reason: options.baselineDirty === true
    ...
    const areas = new Set(changePaths.map((path) => path.includes("/") ? (path.split("/")[0] ?? "(root)") : "(root)"));
    if (changePaths.length >= BROAD_FILE_COUNT || areas.size >= BROAD_AREA_COUNT)
        signals.push(signal("WARNING", "SCOPE_BROAD_FILE_SPREAD", "*", `${changePaths.length} files across ${areas.size} top-level areas changed`, `The deterministic breadth threshold fired at ${BROAD_FILE_COUNT} files or ${BROAD_AREA_COUNT} top-level areas. Patch size alone is not a failure condition.`, changePaths.slice(0, MAX_EVIDENCE_LINES)));
    signals.sort((left, right) => `${left.file}\0${left.code}`.localeCompare(`${right.file}\0${right.code}`));
    return {
        available: true,
...
//# sourceMappingURL=scope-integrity.js.map