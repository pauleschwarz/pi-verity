...
async function gitText(root, ref, path) {
    try {
 ...
 */
export async function summarizeTestDelta(options) {
    const testFiles = options.changedFiles.filter(isTestPath);
    const usable = options.baselineDirectory !== undefined ||
        (options.baselineRef !== undefined && options.baselineDirty !== true);
    if (!usable)
        return { ...EMPTY_TEST_DELTA };
    if (testFiles.length === 0)
        return { ...EMPTY_TEST_DELTA, available: true };
    const delta = { ...EMPTY_TEST_DELTA, available: true };
    for (const path of testFiles) {
        const before = options.baselineDirectory !== undefined
        ...
        if (before === null) {
            delta.files_added += 1;
            if (after !== null)
                delta.assertions_added += count(ASSERTION, after);
            continue;
        }
        if (after === null) {
        ...
        if (assertionsAfter >= assertionsBefore)
            ...
            delta.assertions_removed += assertionsBefore - assertionsAfter;
        delta.skips_added += Math.max(0, count(SKIP, after) - count(SKIP, before));
        delta.suppressions_added += Math.max(0, count(SUPPRESSION, after) - count(SUPPRESSION, before));
    }
    delta.weakened =
...
export function formatTestDelta(delta) {
    if (!delta.available)
        return "";
    const parts = [];
    if (delta.files_added > 0)
        parts.push(`${delta.files_added} added`);
    if (delta.files_modified > 0)
        parts.push(`${delta.files_modified} modified`);
    if (delta.files_deleted > 0)
        parts.push(`${delta.files_deleted} deleted`);
    if (parts.length === 0)
        return "";
    return [
        parts.join(" · "),
