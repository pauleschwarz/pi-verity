...
const SOURCE_EXTENSION = /^\.(?:tsx?|jsx?|mjs|cjs|vue|svelte|astro|html?|css|scss|sass|less|md|mdx|json|ya?ml|py|go|rs|rb|php|java|kt|swift|ex|erb|hbs|twig|liquid)$/i;
export const EMPTY_EFFECT_EVIDENCE = { claims: [] };
        ...
        try {
            ...
            if (observed !== null) {
                return {
                    ...base,
                    observed,
                    status: observed === claim.expected ? "RUNTIME_OBSERVED" : "RUNTIME_CONTRADICTED",
                };
            }
        }
        catch {
...
export async function proveEffects(options) {
...
export function contradictedEffects(evidence) {
    return evidence.claims.filter((claim) => claim.status === "SOURCE_CONTRADICTED" || claim.status === "RUNTIME_CONTRADICTED");
}
export function uncheckedEffects(evidence) {
    return evidence.claims.filter((claim) => claim.status === "UNCHECKED");
}
//# sourceMappingURL=effect-proof.js.map