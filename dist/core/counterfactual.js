...
function antiGaming(file, before, after) {
    ...
    if (count(UNCONDITIONAL, after) > count(UNCONDITIONAL, before) &&
...
export function patchPolarity(classification) {
    if (classification === "PROVEN_REGRESSION")
        return "PROVEN";
    if (classification === "NON_DISCRIMINATING_TEST")
        return "NON_DISCRIMINATING";
    return "UNDETERMINED";
}
function hasStructuralBaselineFailure(result) {
...
function resultClassification(baseline, candidate) {
    ...
    if (baseline.exit_code === 0)
        return {
            ...
            diagnosis: "Candidate test passes against both baseline and candidate implementations",
        };
    if (hasStructuralBaselineFailure(baseline))
        return {
            classification: "TEST_NOT_PORTABLE",
            diagnosis: "Candidate test cannot execute on the baseline because it references candidate-only code",
        };
    return {
...
export async function runCounterfactual(options) {
    ...
    const allTests = [...new Set([...baselineFiles, ...candidateFiles])].sort((left, right) => left.localeCompare(right));
    const changedTests = [];
    const signals = [];
    ...
    const implementationChanges = await Promise.all(implementationFiles.map(async (file) => {
        const before = await text(join(options.baseline.directory, file));
        const after = await text(join(options.root, file));
        return before !== after;
    }));
    ...
    if (selectedTests.length === 0 && !implementationChanged)
        return null;
    const baseEvidence = {
