...
test("current schema matches the source schema version", () => {
  ...
  assert.equal(v4.properties.schema_version.const, SCHEMA_VERSION);
});

test("current schema declares the version 4 evidence contract", () => {
  assert.equal(v4.required.includes("test_delta"), true);
  assert.equal(v4.required.includes("effect_evidence"), true);
  assert.equal(v4.$defs.commandResult?.required?.includes("narrowing"), true);
  assert.equal(v4.$defs.counterfactualEvidence?.required?.includes("narrowing"), true);
  assert.deepEqual(v4.$defs.effectObservation?.required, [
    "claim_id",
    "kind",
    "expected",
    "observed",
  ...
  assert.deepEqual(v4.$defs.effectObservation?.properties?.status.enum, [
    "SOURCE_OBSERVED",
    "RUNTIME_OBSERVED",
    "UNCHECKED",
    "SOURCE_CONTRADICTED",
  ...
  assert.equal(
    v4.$defs.scopeSignal?.properties?.code.enum?.includes("SCOPE_TEST_RENAMED"),
    true,
  );
});
